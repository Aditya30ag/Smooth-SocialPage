    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-8rem)] 
          ...
          {/* Messages Container */}
          <div className="flex-1 h-1/2 overflow-y-auto p-4 scrollbar-thin scrollbar-track-transparent">
            {messages.map((message, index) => (
              <ChatMessage key={index} message={message.text} isUser={message.isUser} />
            ))}
          ...
          {/* Input Area */}
          <div className="p-4 border-t border-slate-800 overflow-auto h-1/3">
            {showContactForm ? (
              <ContactForm 
                onSubmit={handleContactFormSubmit}
                onCancel={() => {
